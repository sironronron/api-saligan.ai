...
<div style="padding: 40px 40px 8px 40px;">
    ...
    <p class="email-copy" style="margin: 0 0 16px 0; font-family: 'Inter', -apple-system, 'Segoe UI', sans-serif; font-size: 16px; line-height: 1.65; color: #5E3122;">
        @if ($reason === 'messages')
            Your free trial of Batayan.ai has <strong style="color: #1D4533;">{{ $remaining }} {{ Str::plural('message', $remaining) }}</strong> remaining across your organization. When they run out, the trial ends.
        @else
            Your free trial of Batayan.ai ends on <strong style="color: #1D4533;">{{ $endsAt?->format('j F Y') }}</strong>.
        @endif
    </p>
