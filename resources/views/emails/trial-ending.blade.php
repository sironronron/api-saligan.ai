...
<div style="padding: 40px 40px 8px 40px;">
    ...
    <h1 style="margin: 0 0 16px 0; font-family: 'Fraunces', Georgia, 'Times New Roman', serif; font-size: 26px; font-weight: 700; color: #1D4533; line-height: 1.3;">
        @if ($reason === 'messages')
            You have <span style="color: #5E3122;">{{ $remaining }} {{ Str::plural('message', $remaining) }}</span> left
        @else
            Your trial ends in <span style="color: #5E3122;">{{ $remaining }} {{ Str::plural('day', $remaining) }}</span>
        @endif
    </h1>
    ...
    <p class="email-copy" style="margin: 0 0 16px 0; font-family: 'Inter', -apple-system, 'Segoe UI', sans-serif; font-size: 16px; line-height: 1.65; color: #5E3122;">
        @if ($reason === 'messages')
            Your free trial of Saligan.AI has <strong style="color: #1D4533;">{{ $remaining }} {{ Str::plural('message', $remaining) }}</strong> remaining across your organization. When they run out, the trial ends.
        @else
            Your free trial of Saligan.AI ends on <strong style="color: #1D4533;">{{ $endsAt?->format('j F Y') }}</strong>.
        @endif
    </p>
    ...
    <p class="email-copy" style="margin: 0 0 24px 0; font-family: 'Inter', -apple-system, 'Segoe UI', sans-serif; font-size: 16px; line-height: 1.65; color: #5E3122;">
        Subscribe to keep your cases, documents, and templates exactly as you left them — nothing is deleted, and your workspace stays put.
    </p>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin: 0 0 32px 0;">
        <tr>
            <td class="email-button" align="center" style="background-color: #1D4533; border-radius: 10px; box-shadow: 0 4px 12px rgba(29, 69, 51, 0.25);">
                <a href="{{ $pricingUrl }}" target="_blank" rel="noopener" style="display: inline-block; padding: 16px 40px; font-family: 'Inter', -apple-system, 'Segoe UI', sans-serif; font-size: 16px; font-weight: 700; color: #F7EAE0; text-decoration: none; letter-spacing: 0.3px;">Choose a plan</a>
            </td>
        </tr>
    </table>
    ...
    <p style="margin: 0 0 32px 0; font-family: 'Inter', -apple-system, 'Segoe UI', sans-serif; font-size: 13px; line-height: 1.5;">
        <a href="{{ $pricingUrl }}" style="color: #1D4533; text-decoration: underline; word-break: break-all;">{{ $pricingUrl }}</a>
    </p>
